...
def main():
    ...
    cocos_root = os.path.abspath(os.path.join(project_root, 'cocos2d-x-tools/frameworks/cocos2d-x/'))
    cxx_generator_root = os.path.abspath(os.path.join(project_root, 'cocos2d-x-tools/frameworks/cocos2d-x/tools/bindings-generator'))
    ...
    config.set('DEFAULT', 'cocos2dx_tools_dir', '%s/cocos2d-x-tools/src' % project_root)
    ...
    try:
        ...
        tolua_root = '%s/cocos2d-x-tools/frameworks/cocos2d-x/tools/tolua' % project_root
        output_dir = '%s/cocos2d-x-tools/src/lua' % project_root

        cmd_args = {'cocos2dx_tools.ini' : ('cocos2d-x-tools', 'lua_cocos2dx_tools_auto')}
        target = 'lua'
        generator_py = '%s/generator.py' % cxx_generator_root
        for key in cmd_args.keys():
            args = cmd_args[key]
            # cfg = '%s/%s' % (tolua_root, key)
            cfg = '%s/cocos2d-x-tools/tools/%s' % (project_root, key)
            print("-" * 40 + cfg)
